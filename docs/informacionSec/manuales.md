...
<div id="upload-form-manuales" class="upload-form-manuales" style="display:none">
  ...
  <div class="form-grid">
    <div class="form-group">
      <label>Título *</label>
      <input type="text" id="input-titulo-m" placeholder="Ej: Manual de Usuario 2026" />
    </div>
    <div class="form-group">
      <label>Descripción</label>
      <input type="text" id="input-desc-m" placeholder="Breve descripción del manual" />
    </div>
    <div class="form-group">
      <label>Archivo PDF *</label>
      <input type="file" id="input-pdf-m" accept=".pdf" />
    </div>
  </div>
...
<div id="manuales-grid" class="cards-grid"></div>
...
.upload-form-manuales h3 { margin: 0 0 16px 0; font-size: 15px; color: #1f2937; border-bottom: none !important; }
...
.upload-msg-manuales { margin-top: 10px; font-size: 13px; padding: 8px 12px; border-radius: 6px; display: none; }
.upload-msg-manuales.ok { background: #dcfce7; color: #166534; display: block; }
.upload-msg-manuales.err { background: #fee2e2; color: #dc2626; display: block; }
.cards-grid {
  display: grid;
...
(function() {
  ...
  function toggleFormManuales() {
    ...
    if (msg) { msg.className = 'upload-msg-manuales'; msg.textContent = ''; }
  }
  ...
  function renderDocs(docs) {
    ...
    if (!docs.length) {
      grid.innerHTML = '<div class="empty-state">📭 No hay manuales todavía. ¡Sube el primero!</div>';
      return;
    }
  ...
  window.subirDoc = function(modulo) {
    ...
    fetch(API, { 
      ...
      .then(function(res) {
        ...
        msg.textContent = '✅ Manual subido correctamente!';
